...
public class HiveBookFontRenderer{
    ...
    private ItemStack itemStack = new ItemStack(Item.appleGold.itemID, 1,0);

    HiveBookFontRenderer()
    {
        this.renderEngine = null;
        this.fontTextureName = null;
    }

    public HiveBookFontRenderer(Minecraft minecraft, GameSettings par1GameSettings, String par2Str, RenderEngine par3RenderEngine, boolean par4)
    {
    ...
    private float renderCharAtPos(int id, char c, boolean par3)
    {
    	if(c == '\t'){
    		...
    		return 0.f;
    	}else if(this.itemMode && c == '>' && this.itemID != 0){
    		GL11.glDisable(GL11.GL_LIGHTING);
            GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
    		this.theGame.renderEngine.bindTexture("/mods/dnd91/minecraft/hivecraft/textures/gui/KnowledgesBG.png");
    		this.drawTexturedModalRect((int)this.posX + 0, (int)this.posY - 12, 0, 202, 25, 25);
    		
            this.itemStack.itemID = this.itemID;
            this.itemStack.setItemDamage(this.itemDMG);
            
            RenderHelper.enableGUIStandardItemLighting();
    		this.itemRender.renderItemAndEffectIntoGUI(this.theGame.fontRenderer, this.theGame.renderEngine, this.itemStack, (int)this.posX + 5, (int)this.posY - 7);
    		GL11.glDisable(GL11.GL_LIGHTING);
        ...
        return c == 32 ? 4.0F : (id > 0 && !this.unicodeFlag ? this.renderDefaultChar(id + 32, par3) : this.renderUnicodeChar(c, par3));
    }
    ...
    private int renderString(String par1Str, int par2, int par3, int par4, boolean par5)
    {
        ...
        else
        {
            ...
            GL11.glColor4f(this.red, this.blue, this.green, this.alpha);
            this.firstX = (float)par2;
            this.posX = (float)par2;
            this.posY = (float)par3 + addNextY;
            addNextY = 0;
